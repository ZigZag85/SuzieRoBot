...
from SuzieRoBot import MONGO_DB_URI
from SuzieRoBot import telethn
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import importlib
        import SuzieRoBot.events

        path = Path(f"SuzieRoBot/modules/{shortname}.py")
        name = "SuzieRoBot.modules.{}".format(shortname)
        ...
        mod.SuzieRoBot = SuzieRoBot
        mod.tbot = telethn
        mod.logger = logging.getLogger(shortname)
        spec.loader.exec_module(mod)
        sys.modules["SuzieRoBot.modules." + shortname] = mod
        print("Successfully imported " + shortname)


path = "SuzieRoBot/modules/*.py"

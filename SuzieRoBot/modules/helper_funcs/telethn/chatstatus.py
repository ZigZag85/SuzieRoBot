from SuzieRoBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from SuzieRoBot import DRAGONS
...
async def Suzie_is_admin(chat_id: int):
    status = False
    Suzie = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Suzie.id == user.id:

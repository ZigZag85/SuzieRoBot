...
import SuzieRoBot.modules.sql.notes_sql as sql
from SuzieRoBot import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from SuzieRoBot.__main__ import DATA_IMPORT
from SuzieRoBot.modules.helper_funcs.chat_status import user_admin
from SuzieRoBot.modules.helper_funcs.alternate import typing_action

# from SuzieRoBot.modules.rules import get_rules
import SuzieRoBot.modules.sql.rules_sql as rulessql

# from SuzieRoBot.modules.sql import warns_sql as warnssql
import SuzieRoBot.modules.sql.blacklist_sql as blacklistsql
from SuzieRoBot.modules.sql import disable_sql as disabledsql

# from SuzieRoBot.modules.sql import cust_filters_sql as filtersql
# import SuzieRoBot.modules.sql.welcome_sql as welcsql
import SuzieRoBot.modules.sql.locks_sql as locksql
from SuzieRoBot.modules.connection import connected
...
def export_data(update, context):
    ...
    with open("SuzieRoBot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("SuzieRoBot{}.backup".format(chat_id), "rb"),
        caption="💾*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `SuzieRoBot-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("SuzieRoBot{}.backup".format(chat_id))  # Cleaning file
